...
- High-end contemporary broadcast for `/tv` (not CRT cosplay, not a YouTube clone): a live ident strap (CC-TV + a giant channel number + a thin voice line + a crawl of official blocks / years on CH 07), ON AIR clock, graphic tuner strip, stage, and hairline lower-third. `/tv` opens on **CH 07** with a public program whose image reads clearly; the ident remains available in its official block. On narrow screens, the program caption flows below the picture so neither competes with the other. The GUIDE starts with six varied public broadcasts, then offers search by title/year/block and a select of verified program blocks. Results are short paged lists, not a 366-row scroll; each row tunes the stage and opens its archive file. House channels keep their smaller example lineups and are explicitly identified as concepts. These are navigation treatments for a future archive, not claims of a complete catalog or published schedule.
